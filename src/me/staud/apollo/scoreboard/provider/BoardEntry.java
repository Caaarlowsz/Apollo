...
public class BoardEntry {
    ...
    public BoardEntry setup() {
        // put your plugin main class shit here
        if (Bukkit.getPluginManager().getPlugin(Apollo.getInstance().getDescription().getName()) != null) {
    ...
    public BoardEntry send(final int position) {
        if (Bukkit.getPluginManager().getPlugin(Apollo.getInstance().getDescription().getName()) != null) {
            ...
            if (this.text.length() > 16) {
                ...
                final String suffix = fix ? this.text.substring(15, this.text.length())
                        : (String.valueOf(ChatColor.getLastColors(prefix))
                        + this.text.substring(16, this.text.length()));
